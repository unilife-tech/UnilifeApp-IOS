...
//
// Reference (http://www.mathiaskoehnke.com) // https://github.com/mkoehnke/LongPressRecordButton
//
...
//        swipeUp = UISwipeGestureRecognizer(target: self, action: #selector(LongPressRecordButton.handleSwipePress(_:)))
//          swipeleft = UISwipeGestureRecognizer(target: self, action: #selector(LongPressRecordButton.handleSwipePress(_:)))
...
//        addTarget(self, action: #selector(LongPressRecordButton.handleSwipePress(_:)), for: UIControl.Event.touchUpInside)
    ...
    @objc fileprivate func handleLongPress(_ recognizer: UILongPressGestureRecognizer) {
        if (recognizer.state == .began) {
            ...
            ///.... if up then y going minus
            ////.... if left then x going minus
            if(location.x > 1) && (location.y < -60)
            {
                ///... up diraction
                buttonSwipeUPContinue()
                
            }else if(location.x < -60) && (location.y > 1)
            {
                /////... diraction left
                buttonSwipeLeftCancel()
            }else
            {
                buttonReleased()
            }
...
//    @objc fileprivate func handleSwipePress(_ recognizer: UISwipeGestureRecognizer) {
...
//       }
    ...
    fileprivate func buttonSwipeUPContinue() {
           if let touchesStarted = touchesStarted , (CACurrentMediaTime() - touchesStarted) >= Double(minPressDuration) {
               self.touchesStarted = nil
               circleLayer.fillColor = circleColor?.cgColor
               delegate?.swipeUPLock(self)
           } else {
               touchesEnded = true
           }
       }
    
    fileprivate func buttonSwipeLeftCancel() {
           if let touchesStarted = touchesStarted , (CACurrentMediaTime() - touchesStarted) >= Double(minPressDuration) {
               self.touchesStarted = nil
               circleLayer.fillColor = circleColor?.cgColor
               delegate?.swipeLeftCancel(self)
           } else {
               touchesEnded = true
           }
       }
